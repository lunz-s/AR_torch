...
if __name__ == '__main__':
    ...
    # Autofill average norm and dual parameters
    if not 'AVERAGE_NOISE_NORM' in parameters.keys():
        AVERAGE_NOISE_NORM = torch.mean(norm(val_gt - val_fbp))
        AVERAGE_NOISE_DUAL = torch.mean(dual(val_gt - val_fbp))
        AVERAGE_IMAGE_NORM = torch.mean(norm(val_gt))
        AVERAGE_DUAL_NORM = torch.mean(norm(val_gt))
